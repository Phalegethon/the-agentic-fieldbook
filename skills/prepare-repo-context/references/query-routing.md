...
- `impact-candidates` follows at most 64 changed symbols and asks one relationship question per changed symbol and direction (`callers` for a changed definition or entry point, `importers` for a changed module or definition), so its cost grows with the change set. It answers in two layers - the change set in `changed` and the candidates in `findings` - so it defaults to 8000 output characters on both surfaces, not the 4000 the single-layer queries use; the two layers share that budget: `changed` keeps up to a third of it, shrinking to `result_identity`, `path`, `qualified_name` and then dropping from the tail with `changed-list-trimmed` and `changed_trimmed_count`, and the candidates take the rest. A trimmed `changed` list means asking `changed-symbols` separately or narrowing the change set with `--path-prefix`.
