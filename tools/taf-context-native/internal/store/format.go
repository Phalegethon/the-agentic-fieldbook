...
func validReferenceFields(record model.Record) bool {
	...
	// The count is encoded as a uint32 and reported as a wire counter, so a
	// wider one would be written truncated or make a result unencodable.
	if record.ReferenceCount < 0 || record.ReferenceCount > model.MaximumReferenceCount || isReference != (record.ReferenceCount >= 1) {
		return false
	}
	if isReference {
