...
For the main handoff, the collector is the sole code/diff inspection. Do not run `ls`, `find`, `rg --files`, `git ls-files`, or direct source-file reads against the target repository before or for it. Immediately resolve tools, references, and the repository, then run the collector.

Let `<skill-dir>` be the directory containing this `SKILL.md`.

1. Select the repository (default: current repository), base ref (`main`), and head ref (`HEAD`). Leave SHA and freshness resolution to the collector; do not replace named refs with SHAs. Capture only explicitly supplied context or developer-test-result file paths. Use `--offline` only when requested; include staged, unstaged, and untracked changes only with explicit working-tree opt-in. If Jira or GitHub access is requested, load `<skill-dir>/references/platform-actions.md` and follow its consent flow.
