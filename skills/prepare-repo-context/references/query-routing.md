...
| What am I about to commit? What does my staged change affect? | `changed-symbols --staged` / `impact-candidates --staged` | Measures the index against `HEAD` exactly as `git commit` would record it, excluding unstaged and untracked edits. Exclusive with `--base`; over MCP, `staged: true`. |
